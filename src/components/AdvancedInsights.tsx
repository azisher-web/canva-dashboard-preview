...
/* ═══════════════════════════════════════════════════════════════════════════
   DATA TYPES — computed server-side, passed as props
   ═══════════════════════════════════════════════════════════════════════════ */

export interface NicheDominanceItem {
  niche: string; zone: string; topCreator: string; dominancePct: number; totalCreators: number;
}
export interface CreatorStrategyItem {
  name: string; count: number; proPct: number; strategy: 'Pro-focused' | 'Free-focused' | 'Mixed'; avgPos: number;
}
export interface BlueKeywordItem {
  keyword: string; count: number; niches: string[];
}
export interface KeywordPositionItem {
  keyword: string; avgPosition: number; count: number;
}
export interface KeywordLengthItem {
  type: 'Head (1 word)' | 'Mid-tail (2-3)' | 'Long-tail (4+)'; total: number; blue: number; red: number;
}
export interface PositionBucketItem {
  range: string; blue: number; yellow: number; red: number;
}
export interface FreeOppScoreItem {
  niche: string; zone: string; score: number; proPct: number; count: number;
}
export interface StyleSatItem {
  style: string; nicheCount: number; totalNiches: number; pct: number;
}
export interface StyleZoneItem {
  style: string; blue: number; yellow: number; red: number;
}
export interface ContrarianItem {
  niche: string; nicheStyle: string; categoryDominant: string;
}
export interface StyleComplexItem {
  niche: string; styleCount: number; diversity: number; dominant: string; dominantPct: number;
}
export interface AssetRarityItem {
  name: string; category: string; nicheCount: number; heroCount: number;
}
export interface UnderservedAssetItem {
  category: string; bluePct: number; totalCount: number;
}

export interface AdvancedInsightsData {
  nicheDominance: NicheDominanceItem[];
  creatorStrategies: CreatorStrategyItem[];
  blueKeywords: BlueKeywordItem[];
  keywordRanking: KeywordPositionItem[];
  keywordLength: KeywordLengthItem[];
  positionBuckets: PositionBucketItem[];
  proFreePosition: { proAvg: number; freeAvg: number };
  sweetSpot: { start: number; end: number; blueCount: number; bluePct: number };
  freeOppScores: FreeOppScoreItem[];
  styleSaturation: StyleSatItem[];
  styleZone: StyleZoneItem[];
  contrarianStyles: ContrarianItem[];
  styleComplexity: StyleComplexItem[];
  assetRarity: AssetRarityItem[];
  underservedAssets: UnderservedAssetItem[];
...
export default function AdvancedInsights({ data, isID = false }: { data: AdvancedInsightsData; isID?: boolean }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        <BODashIcon name="sparkles" size={20} color="var(--accent)" />
        ...
        <div className="card2" style={{ padding: 20 }}>
          ...
          <div style={{ maxHeight: 420, overflowY: 'auto' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {data.creatorStrategies.map((c, i) => (
                <div key={i} style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: 20, alignItems: 'start' }}>
        ...
        {/* B3: Keyword → Position Correlation */}
        <div className="card2" style={{ padding: 20 }}>
      ...
      {/* B4: Keyword Length Distribution */}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: 20, alignItems: 'start' }}>
        ...
        {/* C2 + C3: Pro/Free Position Bias + Sweet Spot */}
      ...
      {data.hasDesignData ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: 20, alignItems: 'start' }}>
          ...
          {/* F3: Underserved Asset Categories */}
          <div className="card2" style={{ padding: 20 }}>
            ...
            {data.underservedAssets.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {data.underservedAssets.map((a, i) => (
